...
import type { IdolCategory } from "./generated/prisma/client.js";
...
function categoryFromBody(input: unknown): IdolCategory {
  const key = Object.keys(input as Record<string, unknown>)[0];
  if (!key) throw new Error("Invalid category");
  return key as IdolCategory;
}

function categoryToJson(category: IdolCategory): Record<string, null> {
...
function inquiryToWire(row: {
  ...
  source: (typeof InquirySource)[keyof typeof InquirySource];
  isRead: boolean;
  isArchived: boolean;
  createdAt: Date;
}) {
...
app.post("/idols", async (req, res) => {
  try {
    ...
    if (body.op === "create" && body.idol) {
      ...
      const row = await prisma.idol.create({
        data: {
          ...
          images: i.images as Prisma.JsonArray,
    ...
    if (body.op === "update" && body.idol) {
      const i = body.idol as typeof body.idol & { id: string };
...
app.post("/inquiries", async (req, res) => {
  try {
    ...
    if (body.op === "submit" && body.inquiry) {
      ...
      const idolId =
        q.idolId != null && q.idolId !== "" ? BigInt(q.idolId) : null;
      const source =
        q.source === "whatsapp" ? InquirySource.whatsapp : InquirySource.website;
